    ...
    int& halfMoveClock, int& fullMoveNumber)
{
    ...
    // Split FEN string into parts using spaces as delimiters
    while (std::getline(fenStream, part, ' '))
    {
        parts.push_back(part); // Add each part to the vector
    }

    // If there are not 6 parts, use a default FEN string
    if (parts.size() != 6)
    {
        fenString = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR b KQkq - 0 1";
        fenStream.clear(); // Clear error state of the stream
        fenStream.str(fenString); // Set the stream to the default FEN string
        parts.clear(); // Clear previous parts
        while (std::getline(fenStream, part, ' '))
        {
            parts.push_back(part); // Add each part of the default FEN string
        }
    }

    // Split board FEN into rows
    std::istringstream rowsStream(parts[0]); // Stream to read rows
    std::vector<std::string> rows; // Stores rows of the board
    while (std::getline(rowsStream, part, '/'))
    {
        rows.push_back(part); // Add each row to the vector
    }
    ...
    for (const std::string& row : rows)
    {
        for (char currentFENChar : row)
        {
            ...
            {
                board.push_back(std::string(1, currentFENChar)); // Add piece to the board
            }
        }
    }
    ...
    halfMoveClock = std::stoi(parts[4]); // Get half-move clock (number of half-moves since last capture or pawn move)
    fullMoveNumber = std::stoi(parts[5]); // Get full-move number (total number of moves)
}
